...
if(c.open(t.method.toUpperCase(),tB(u,t.params,t.paramsSerializer),!0),// Set the request timeout in MS
...
function e5(t,e){let r=document.getElementById("booksList");if(r.innerHTML="",t&&t.length>0)t.forEach(t=>{let e=document.createElement("li");e.className="list-container",e.innerHTML=`
      ...
      `,e.addEventListener("click",async()=>{let e=document.getElementById("bookCardContainer");e.innerHTML="";try{let r=await eT(t._id),n=e$(r);e.appendChild(n)}catch(t){console.error(t)}}),r.appendChild(e)});else{let t=document.createElement("p");t.textContent="No books found in this category.",r.appendChild(t)}"All categories"===e?r.classList.replace("books_category-list","books-list"):r.classList.replace("books-list","books_category-list")}e1.addEventListener("click",function(){let t=document.getElementById("charitiesSlider"),e=t.children[0].offsetHeight;if(e0){t.style.transform="translateY(0)";for(let e=0;e<6;e++)t.children[e].style.visibility="visible";e2("up")}else{t.style.transform=`translateY(-${5*e}px)`;for(let e=0;e<3;e++)t.children[e].style.visibility="hidden";e2("down")}e0=!e0}),document.addEventListener("scroll",function(){var t;t=document.getElementById("scrollBtn"),document.body.scrollTop>20||document.documentElement.scrollTop>20?t.style.display="block":t.style.display="none"}),document.getElementById("scrollBtn").addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})}),document.getElementById("charitiesSlider"),eB().then(t=>{//console.log('Categories Received:', categoriesData);
!// lista kategorii
